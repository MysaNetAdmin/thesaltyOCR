...
void print_grill(struct queue* queue, size_t height_max, size_t width_max){

	SDL_Surface* res = SDL_CreateRGBSurface(0,width_max,height_max,32,0,0,0,0);
	size_t z = 0;
	while(!queue_is_empty(queue))
	{
		...
		for(x = 0; x < backline - frontline ; x++ ){
			for(size_t y = 0; y < height; y++){
				
Uint32 pix = getpixel(img,x,y);
				putpixel(res,x + z,y,pix);
			}
		}
		printf("%zu",z);
	}
...
const char usage[] =
" U FORGET THE FUCKING FUNCTION TO CALL\n"
"\t\t0: black and white\n"
"\t\t1: show black lines between lines\n"
"\t\t2: display lines of the image\n"
"\t\t3: display letters of the image\n";
	...
	if(argc <= 1)
		...
		switch(f){
			...
			case 1:
				...
				break;
			case 2:
				struct queue* queue = horizon(black_n_white(ver),1);
				free(queue);
				break;
			case 3:
				print_grill(horizon(black_n_white(ver),0),height,width);
				break;
			case 4:
				display_image(init_SDL(400,400));
				break;
		}
